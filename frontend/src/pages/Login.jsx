...
export default function Login() {
  ...
  const submit = async e => {
    ...
    const res = tab === 'login' ? login(f.email, f.password) : signup(f.name, f.email, f.password);
  ...
  const googleLogin = () => {
    const res = login('demo@xeno.com', 'demo123');
    if (res.ok) nav('/dashboard');
  };
    ...
    <div className="auth-page" data-theme={theme}>
      ...
      <main className="auth-shell">
        ...
        >
          ...
          <p>
            Sign in to manage customer data, launch AI-assisted campaigns, and track delivery analytics in real time.
          </p>
        ...
        >
          ...
          <form onSubmit={submit} className="auth-form">
            ...
            {err && <motion.div className="err-box" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>{err}</motion.div>}

            <button className="auth-submit" type="submit" disabled={busy}>
              {busy ? <span className="mini-spinner" /> : null}
              {busy ? (tab === 'login' ? 'Signing in...' : 'Creating account...') : (tab === 'login' ? 'Login' : 'Create account')}
            </button>
          </form>
